	...
	private GUILogin() {
		...
		lblNewUser.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				GUIRegistrationReader.main(GUILogin.frame);
			}
			...
			public void actionPerformed(ActionEvent e) {
				if (lblErrorID.getText().length() == 0 && lblErrorPassword.getText().length() == 0) {
					Login login = new Login(textFieldID.getText().trim(),
							new String(passwordFieldPassword.getPassword()), "Reader");
					User user = Proxy.instance().login(Operation.LOGIN_USER, login);
					Login.setUserLogged(user);
					if (user == null) {
						lblError.setVisible(true);
					} else {
						boolean userActive = user.getActivationCode() == null;
						if (!userActive) {
						...
						if (user != null) {
							boolean tmpPwd = user.isTemporaneyPassword();
							if (userActive && tmpPwd) {
								...
								while (doCycle) {
									...
									JLabel lblerrorFirstPwd = new JLabel("Questo campo è vuoto!");
									lblerrorFirstPwd.setForeground(Color.RED);
									lblerrorFirstPwd.setFont(new Font("Tahoma", Font.PLAIN, 16));
									JLabel lblConfirmPassword = new JLabel("Confermare nuova password:");
									lblConfirmPassword.setFont(new Font("Tahoma", Font.PLAIN, 16));
									JPasswordField confirmPassword = new JPasswordField();
									confirmPassword.setFont(new Font("Tahoma", Font.PLAIN, 16));
									JLabel lblerrorConfirm = new JLabel("Questo campo è vuoto!");
									lblerrorConfirm.setForeground(Color.RED);
									lblerrorConfirm.setFont(new Font("Tahoma", Font.PLAIN, 16));
									jPasswordField.addFocusListener(new FocusAdapter() {
										@Override
										public void focusLost(FocusEvent e) {
											String newPwd = new String(jPasswordField.getPassword());
											String confirmNewPwd = new String(confirmPassword.getPassword());
											String errorNewPwd = Utility.verifyNotEmptyField(newPwd)
													+ Utility.verifyPasswordLength(newPwd)
													+ Utility.verifyLettersAndNumbers(newPwd)
													+ Utility.verifyConfirmPassword(newPwd, confirmNewPwd);
											lblerrorFirstPwd.setText(errorNewPwd.split("-")[0]);
									...
									confirmPassword.addFocusListener(new FocusAdapter() {
										...
										public void focusLost(FocusEvent e) {
											...
											String errorNewPwd = Utility.verifyNotEmptyField(newPwd)
													...
													+ Utility.verifyLettersAndNumbers(newPwd)
													+ Utility.verifyConfirmPassword(newPwd, confirmNewPwd);
											lblerrorConfirm.setText(errorNewPwd.split("-")[0]);

										}
									});
									Object[] objects = { lblNewPassword, jPasswordField, lblerrorFirstPwd,
											lblConfirmPassword, confirmPassword, lblerrorConfirm };
									int res = JOptionPane.showConfirmDialog(null, objects, "Modifica password generata",
											JOptionPane.OK_CANCEL_OPTION);
									if (res == JOptionPane.OK_OPTION) {
										if (lblerrorFirstPwd.getText().length() == 0
												&& lblerrorConfirm.getText().length() == 0) {
											String responsePwd = Proxy.instance().modifyPassword(
													new String(passwordFieldPassword.getPassword()),
													new String(jPasswordField.getPassword()));
											if (responsePwd != null && responsePwd.equals("Operation completed")) {
												user.setTemporaneyPassword(false);
												tmpPwd = false;
												doCycle = false;
											}
										} else {
											JOptionPane.showMessageDialog(null,
													"Correggere i dati errati prima di procedere!", "Errore!",
													JOptionPane.ERROR_MESSAGE);
										}
									} else {
										Proxy.instance().logout();
