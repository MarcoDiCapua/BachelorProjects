...
/**
 * This class create the modify reader interface
 *
 ...
 * @version 1.0
 */
public class GUIModifyReader extends JDialog {
	...
	private JComboBox<Classification> comboBoxClassification;
	private JComboBox<String> comboBoxClassroom;
	private JComboBox<String> comboBoxDepartment;
	private static GUIModifyReader dialog;
	...
	public static void main(JFrame frame) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					dialog = new GUIModifyReader(frame);
	...
	private GUIModifyReader(JFrame frame) {
		super(frame, true);
		setResizable(false);
		...
		gbl_panelGridFields.columnWidths = new int[] { 118, 231, 116, 234, 0 };
		gbl_panelGridFields.rowHeights = new int[] { 1, 26, 26, 26, 26, 26, 20, 0 };
		...
		gbc_lblConfirmEmail.anchor = GridBagConstraints.WEST;
		...
		comboBoxClassification = new JComboBox<Classification>();
		...
		gbc_comboBoxClassroom.anchor = GridBagConstraints.NORTHWEST;
		gbc_comboBoxClassroom.insets = new Insets(0, 0, 5, 5);
		gbc_comboBoxClassroom.gridx = 1;
		gbc_comboBoxClassroom.gridy = 5;
		...
		gbc_comboBoxDepartment.anchor = GridBagConstraints.NORTHWEST;
		gbc_comboBoxDepartment.insets = new Insets(0, 0, 5, 0);
		gbc_comboBoxDepartment.gridx = 3;
		gbc_comboBoxDepartment.gridy = 5;
		...
		btnModifyPassword.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				GUIModifyPassword.main(GUIModifyReader.dialog);
			}
		});
		...
		buttonDeleteUser.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				GUIDeleteUser.main(GUIModifyReader.dialog);
			}
		});
			...
			public void actionPerformed(ActionEvent e) {
				if (lblErrorEmail.getText().length() == 0 && lblErrorConfirmEmail.getText().length() == 0
					...
					if (!Login.getUserLogged().getTelephoneNumber().equals(textFieldPhone.getText())
							|| !Login.getUserLogged().getClassification()
									.equals((Classification) comboBoxClassification.getSelectedItem())
							|| (Login.getUserLogged().getClassification().equals((Classification.STUDENT))
									&& (!Login.getUserLogged().getYear_class().split("-")[0]
											.equals(comboBoxClassroom.getSelectedItem().toString())
											|| !Login.getUserLogged().getYear_class().split("-")[1]
													.equals(comboBoxDepartment.getSelectedItem().toString())))
						...
						String reposnseClassification = "";
						if (!Login.getUserLogged().getClassification()
								.equals((Classification) comboBoxClassification.getSelectedItem())
								|| !Login.getUserLogged().getYear_class().split("-")[0]
										.equals(comboBoxClassroom.getSelectedItem().toString())
								|| !Login.getUserLogged().getYear_class().split("-")[1]
										.equals(comboBoxDepartment.getSelectedItem().toString())) {
							Login.getUserLogged()
									.setClassification((Classification) comboBoxClassification.getSelectedItem());
							Login.getUserLogged().setYear_class(comboBoxClassroom.getSelectedItem().toString() + "-"
									+ comboBoxDepartment.getSelectedItem().toString());

							reposnseClassification = Proxy.instance().modifyClassification(
									Login.getUserLogged().getClassification(),
									comboBoxClassroom.getSelectedItem().toString() + "-"
											+ comboBoxDepartment.getSelectedItem().toString());

							if (reposnseClassification != null) {
								if (reposnseClassification.equals("Operation completed")) {
									JOptionPane.showMessageDialog(null,
											"L'inquadramento è stato modificato correttamente",
											"Inquadramento modificato!", JOptionPane.INFORMATION_MESSAGE);
								} else {
									JOptionPane.showMessageDialog(null,
											"Non è stato possibile modificare l'inquadramento come richiesto! (Errore: "
													+ reposnseClassification + ")",
											"Errore!", JOptionPane.ERROR_MESSAGE);
								}
							}
						}
						...
						if (responseTelephone != null && reposnseClassification != null
								&& !Login.getUserLogged().getEmail().equals(textFieldEmail.getText())) {
	...
	private void fillInFields() {
		...
		comboBoxClassification.setSelectedItem(Login.getUserLogged().getClassification());
		if (Login.getUserLogged().getClassification().equals(Classification.STUDENT)) {
			comboBoxClassroom.setSelectedItem(Login.getUserLogged().getYear_class().split("-")[0]);
			comboBoxDepartment.setSelectedItem(Login.getUserLogged().getYear_class().split("-")[1]);
		}
	}
}
