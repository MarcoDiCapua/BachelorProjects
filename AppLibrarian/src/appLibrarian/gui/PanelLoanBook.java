...
/**
 * This class create the loan book panel
 *
...
public class PanelLoanBook extends JPanel {
	...
	public PanelLoanBook() {
				...
				if (tableLoans.getRowCount() != 0) {
					for (int i = 0; i < tableLoans.getRowCount(); i++) {
						...
						if (bookToBook.getBookings() == 0) {
							...
							JSpinner spnSelectDay = new JSpinner(new SpinnerNumberModel(new Integer(30), new Integer(1),
									new Integer(30), new Integer(1)));
							spnSelectDay.setFont(new Font("Tahoma", Font.PLAIN, 16));
							Object[] ob = { lblInsertDay, spnSelectDay };
							int selectedOption = JOptionPane.showConfirmDialog(null, ob, "Inserire data restituzione",
									JOptionPane.OK_CANCEL_OPTION);
							GregorianCalendar returnDate = new GregorianCalendar();
							if (selectedOption == JOptionPane.OK_OPTION) {
								returnDate.add(Calendar.DAY_OF_MONTH, (int) spnSelectDay.getValue());
								Loan newLoan = new Loan(bookToBook, Login.getUserReaderLogged(),
								...
								if (result != null) {
									if (result.equals("Operation completed")) {
										...
										JOptionPane.showMessageDialog(null,
												"Il prestito del libro con codice isbn: " + bookToBook.getIsbn()
							...
							Booking newBooking = new Booking(bookToBook, Login.getUserReaderLogged(),
									new GregorianCalendar());
							String result = Proxy.instance().bookingBook(newBooking);
							if (result.equals("Operation completed")) {
								Login.getUserReaderLogged().getBookingBooks().add(newBooking);
								JOptionPane.showMessageDialog(null,
		...
		panelFilters.setPreferredSize(new Dimension(300, 260));
		...
		gbl_panelFilters.rowHeights = new int[] { 35, 26, 45, 26, 45, 26, 0 };
		...
		tableBooks.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				if ((boolean) tableBooks.getValueAt(tableBooks.getSelectedRow(), 0)) {
					...
					((BookingTableModel) tableLoans.getModel()).addBooking(selectedBook, Login.getUserReaderLogged());
				} else {
