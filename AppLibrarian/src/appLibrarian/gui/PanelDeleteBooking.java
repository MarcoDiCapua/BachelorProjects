...
/**
 * This class create the delete booking panel
 *
...
public class PanelDeleteBooking extends JPanel {
	...
	public PanelDeleteBooking() {
				...
				for (int i = 0; i < tableBookings.getRowCount(); i++) {
					...
					if (result != null) {
						if (result.equals("Operation completed")) {
							Login.getUserReaderLogged().getBookingBooks().remove(bookingToDelete);
		...
		scrollPaneLoans.setPreferredSize(new Dimension(469, 178));
		panelLoans.add(scrollPaneLoans);

		JPanel panelLogo = new JPanel();
		panelLogo.setBorder(null);
		panelLogo.setPreferredSize(new Dimension(10, 315));
		...
		panelCenter.setMinimumSize(new Dimension(10, 90));
		...
		scrollPaneBooks.setPreferredSize(new Dimension(10, 178));
